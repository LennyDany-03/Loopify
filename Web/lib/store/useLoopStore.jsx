...
const useLoopStore = create((set, get) => ({
  ...
  fetchLoops: async () => {
    ...
    try {
      ...
      set({ error: err.response?.data?.detail || "Failed to load loops.", isLoading: false });
    }
  },
  ...
  createLoop: async (data) => {
    try {
      ...
      return { success: false, error: err.response?.data?.detail || "Failed to create loop." };
    }
  },
  ...
  updateLoop: async (id, data) => {
    try {
      ...
      return { success: false, error: err.response?.data?.detail || "Failed to update loop." };
    }
  },
  ...
  deleteLoop: async (id) => {
    try {
      ...
      return { success: false, error: err.response?.data?.detail || "Failed to delete loop." };
    }
  },
  ...
  checkinLoop: async (loopId, value = null, note = null) => {
    try {
      ...
      // Update streak stats on the loop in store
      set((state) => ({
        loops: state.loops.map((l) =>
          l.id === loopId
            ? { ...l, current_streak: streak.current_streak, best_streak: streak.best_streak, total_checkins: streak.total_checkins }
            : l
        ),
        // Mark loop as done today
        todayCheckins: { ...state.todayCheckins, [loopId]: res.data.checkin },
      }));

      return { success: true, streak };
    } catch (err) {
      return { success: false, error: err.response?.data?.detail || "Failed to log checkin." };
    }
  },
